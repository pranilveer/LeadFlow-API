...
router.get("/", async (req, res) => {
  try {
    ...
    const users = await User.find({ organization: orgId }).sort({ createdAt: 1 });
    const leads = await Lead.find({ organization: orgId }).select("addedBy");
...
router.post("/", async (req, res) => {
  try {
    ...
    const user = await User.create({
      organization: orgId, username: username.trim(), password: hashed, name: name.trim(),
    ...
    await Activity.create({ organization: orgId, type: "user", message: `User "${user.name}" created.`, user: req.user.username });
  ...
  try {
    ...
    if (role && role !== user.role) {
      ...
      if (user.role === "admin" && role !== "admin") {
        const adminCount = await User.countDocuments({ organization: orgId, role: "admin" });
    ...
    if (email !== undefined && email !== user.email) {
      const emailTaken = await User.findOne({ organization: orgId, _id: { $ne: user._id }, email: { $regex: new RegExp(`^${email.trim()}$`, "i") } });
      if (emailTaken) return res.status(400).json({ error: "Email already exists." });
      user.email = email;
    }
    ...
    await user.save();
    await Activity.create({ organization: orgId, type: "user", message: `User "${user.name}" updated.`, user: req.user.username });
    res.json(user);
  } catch (err) {
    console.error(err);
    res.status(400).json({ error: err.message });
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    await Activity.create({ organization: orgId, type: "user", message: `User "${user.name}" deleted.`, user: req.user.username });
